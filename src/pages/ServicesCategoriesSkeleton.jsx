...
const ServicesCategoriesSkeleton = () => {
  ...
  const { data: response, isLoading, error } = useGetNavDropdownItemByCategoryQuery(categorySlug);
              ...
              {service.subcategories && service.subcategories.length > 0 ? (
                service.subcategories.map((sub, idx) => (
                  <TiltCard
                    ...
                    onClick={() => navigate(`/services/${encodeURIComponent(service.categorySlug)}/${encodeURIComponent(typeof sub === 'string' ? sub : sub.slug)}`)}
                    delay={0.1 * idx}
                  />
                ))
              ) : (
