...
import { useGetServicesDetailTwoQuery, useUpdateServicesDetailTwoMutation, useGetNavDropdownItemsQuery, useUploadServiceImageMutation } from '../../redux/api';

const ServicesDetailTwoForm = () => {
    const navigate = useNavigate();
    // When opened from the Category Manager ("Add/Edit Detail Page" button), these come
    // straight from the URL so no manual picking is needed: /admin/servicesDetailTwo/:categorySlug/:subcategorySlug
    const { categorySlug: routeCategorySlug, subcategorySlug: routeSubcategorySlug } = useParams();
    const isLockedToRoute = Boolean(routeCategorySlug && routeSubcategorySlug);
    ...
    const { data: pageDataResp, isLoading: isFetching, isSuccess, isFetching: isFetchingData } = useGetServicesDetailTwoQuery(
        { categorySlug: selectedCategorySlug, subcategorySlug: selectedSubcategorySlug },
        { skip: !selectedCategorySlug || !selectedSubcategorySlug }
    );
    ...
    }, [isSuccess, pageDataResp, selectedCategorySlug, selectedSubcategorySlug]);
    ...
    const handleImageUpload = async (e, callback) => {
        ...
        try {
            ...
            const result = await uploadServiceImage(uploadData).unwrap();
            if (result.success) {
                callback(result.url);
    ...
    const selectedCategoryData = availableCategories.find(c => c.categorySlug === selectedCategorySlug);
    const availableSubcategories = selectedCategoryData?.subcategories || [];
    const selectedSubcategoryData = availableSubcategories.find(s => s.slug === selectedSubcategorySlug);
    ...
    return (
        <div style={{
            ...
            <p style={{ color: 'rgba(255,255,255,0.6)', marginBottom: '32px' }}>
                {isLockedToRoute
                    ? 'Fill in this subcategory\'s detail page content below.'
                    : 'Select a category and subcategory to manage its specific content.'}
            </p>

            {isLockedToRoute ? (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, marginBottom: '32px', background: 'rgba(59,130,246,0.08)', padding: '18px 24px', borderRadius: '14px', border: '1px solid rgba(59,130,246,0.25)' }}>
                    <div>
                        <div style={{ fontSize: 12, letterSpacing: 1.5, textTransform: 'uppercase', color: '#60a5fa', marginBottom: 4 }}>Editing</div>
                        <div style={{ fontSize: 18, fontWeight: 700, color: '#fff' }}>
                            {selectedCategoryData?.categories || routeCategorySlug} <span style={{ color: 'rgba(255,255,255,0.4)' }}>→</span> {selectedSubcategoryData?.name || routeSubcategorySlug}
                        </div>
                    </div>
                    <button type="button" onClick={() => navigate('/admin/nav-dropdowns')} style={{ padding: '8px 16px', background: 'rgba(255,255,255,0.08)', border: 'none', borderRadius: '6px', color: '#fff', cursor: 'pointer' }}>
                        ← Back to Categories
                    </button>
                </div>
            ) : (
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        >
                            ...
                            {availableCategories.map(cat => (
                                <option key={cat._id} value={cat.categorySlug}>{cat.categories}</option>
                            ))}
                        </select>
                    </div>
                    <div style={{ flex: 1 }}>
                    ...
                    <div style={{ background: 'rgba(255,255,255,0.02)', padding: '24px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)' }}>
                        {activeTab === 'hero' && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                ...
                                <div>
                                    <label style={{ display: 'block', marginBottom: '6px', color: '#a1a1aa' }}>Hero Image</label>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                        {formData.heroimg && (
                                            <img src={formData.heroimg} alt="Hero Preview" style={{ width: '150px', height: '100px', objectFit: 'cover', borderRadius: '8px', border: '1px solid #3b82f6' }} />
                                        )}
                                        <input name="heroimg" value={formData.heroimg || ''} onChange={handleChange} style={inputStyle} placeholder="Or enter URL manually..." />
                                        <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, (url) => setFormData(prev => ({ ...prev, heroimg: url })))} style={inputStyle} disabled={isUploadingImg} />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    {formData.techStack.map((item, idx) => (
                                        <div key={idx} style={itemBoxStyle}>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', width: '100%' }}>
                                                {item.iconUrl && <img src={item.iconUrl} alt="Icon Preview" style={{ width: '40px', height: '40px', objectFit: 'contain' }} />}
                                                ...
                                                <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, (url) => handleUpdateObject('techStack', idx, 'iconUrl', url))} style={inputStyle} disabled={isUploadingImg} />
                                            </div>
                                ...
                                <div style={sectionStyle}>
                                    ...
                                    {formData.universeCards.map((item, idx) => (
                                        <div key={idx} style={itemBoxStyle}>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', width: '100%' }}>
                                                {item.iconUrl && <img src={item.iconUrl} alt="Icon Preview" style={{ width: '40px', height: '40px', objectFit: 'contain' }} />}
                                                <input placeholder="Icon URL" value={item.iconUrl} onChange={(e) => handleUpdateObject('universeCards', idx, 'iconUrl', e.target.value)} style={inputStyle} />
                                                <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, (url) => handleUpdateObject('universeCards', idx, 'iconUrl', url))} style={inputStyle} disabled={isUploadingImg} />
                                            </div>
                                            <button type="button" onClick={() => handleRemoveObject('universeCards', idx)} style={deleteBtnStyle}>Delete</button>
                                        </div>
                                    ))}
                                    {formData.universeCards.length < 4 && (
                                        <button type="button" onClick={() => handleAddObject('universeCards', { title: '', desc: '', iconUrl: '' })} style={addBtnStyle}>+ Add Universe Card</button>
                                    )}
                                </div>
                        ...
                        {/* --- CONTACT TAB CONTENT (Commented Out) ---
                        {activeTab === 'contact' && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                <div style={{ padding: '16px', background: 'rgba(0,0,0,0.2)', borderRadius: '8px' }}>
